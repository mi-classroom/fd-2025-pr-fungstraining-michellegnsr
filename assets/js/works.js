const finishedWorks = [
...

    {
        ...
        "partner": "",
        "partnerUrl": "",
        "image": "/assets/uploads/final-blockdown.webp",
        "relatedFolder": "",
...

    {
        ...
        "partner": "",
        "partnerUrl": "",
        "image": "/assets/uploads/c.noss_a_lucas_cranach_exibition_with_paintings_which_are_prese_63eaa10b-fca7-461f-9ea0-ac4dac887979.jpg",
        "relatedFolder": "",
    ...
    {
        "title": "Can I CAI? Entwicklung einer Webapp zur Überprüfung von Veröffentlichungsprozessen auf Kompatibilität mit aufkommenden Standards der Content Authenticity Initiative zur Auszeichnung von authentischen Medien im Web.",
        "author": "Nils Polarek",
...
function stripHtml(html) {
    // Entfernt alle HTML-Tags wie <p>, <strong>, etc.
  ...
  const workList = document.querySelector('[data-js-finished-works]');
  
  // mit Hilfe vom Copiloten erstellt
  finishedWorks
    .forEach(work => {
      const li = document.createElement('li');
      li.classList.add('work-overview-item');
  
      const rawAbstract = stripHtml(work.abstract);
      const shortAbstract = truncateText(rawAbstract, 200); // max 200 Zeichen
  
      li.innerHTML = `
        <h3><a href="${work.url}">${work.title}</a></h3>
        <div class="meta">
          <span><strong>Autor:</strong> ${work.author}</span> |
          <span><strong>Typ:</strong> ${work.type}</span> |
          <span><strong>Datum:</strong> ${new Date(work.date).toLocaleDateString('de-DE')}</span>
        </div>
        <div class="abstract">${shortAbstract}</div>
      `;
